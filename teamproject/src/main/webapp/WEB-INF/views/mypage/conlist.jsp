...
<ul class="my_gnb">
         ...
         <div class="title_wrap"><h2 class="tit_txt">주문현황</h2></div> 
			...
			<c:when test="${empty list}">
  				...
  				<c:otherwise>
								...
								<tbody>
									<tr>
									<td><c:out value="${list.q_type}"/></td>
									<td><c:out value="${list.q_name}"/></td>
									<td><c:out value="${list.user_id}"></c:out></td>
									<td><c:out value="${list.q_regdate}"></c:out></td>
									</tr>	
								</tbody>								
								</c:forEach>								
								</c:otherwise>
		...
		<ul class="paging">
    		<c:if test="${paging.prev}">
        		<span><a href="${contextPath}/mypage_conlist.do?page=${paging.startPage-1}">이전</a></span>
    		</c:if>
    		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
    		
        		<span><a href="${contextPath}/mypage_conlist.do?page=${num}">${num}</a></span>
        		
    		</c:forEach>
    		<c:if test="${paging.next && paging.endPage>0}">
        		<span><a href="${contextPath}/mypage_conlist.do?page=${paging.endPage+1}">다음</a></span>
    		</c:if>
       </ul>
